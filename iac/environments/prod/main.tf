...
locals {
  ...
  # Environment-specific domain configuration (production uses main domain)
  full_domain = "${local.subdomain}.${local.domain_name}"
  api_domain  = "api.${local.subdomain}.${local.domain_name}"
...
module "lambda" {
  ...
  # IAM Security Settings (stricter for production)
...
module "api_gateway" {
  ...
  cors_configuration = {
    ...
    allow_origins     = ["https://${local.full_domain}"]  # Production: only allow production domain
    expose_headers    = []
    max_age          = 86400
  }
...
module "frontend" {
  ...
  certificate_arn   = module.security.certificate_arn
  route53_zone_id   = module.security.route53_zone_id
  price_class      = "PriceClass_All"  # Production: global distribution
  tags             = local.common_tags
