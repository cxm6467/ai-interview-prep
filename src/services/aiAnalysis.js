...
export class AIAnalysisService {
    ...
    static async callOpenAI(prompt, maxTokens = 1000) {
        try {
...
Return only valid JSON without any additional text or formatting.`;
    ...
    static async analyzeJobDescription(jobText) {
        const prompt = `
...
Return only valid JSON without any additional text.`;
    ...
    static async generateInterviewQuestions(resume, job) {
        const prompt = `
...
Make questions specific to their background at ${resume.experience[0]?.company} and the ${job.title} role.`;
    ...
    static async generatePresentationTopics(resume, job) {
        const prompt = `
...
Make topics highly relevant to their actual experience and the target role.`;
    ...
    static async calculateATSScore(resume, job) {
        const prompt = `
...
Be specific about what keywords are present vs missing and why.`;
    ...
    static async generateInterviewResponse(userMessage, questionContext, resumeData, conversationHistory) {
        const prompt = `
...
Response:`;
    ...
    static getMockJobDescription(jobText) {
        return {
